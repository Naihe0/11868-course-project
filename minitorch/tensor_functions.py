...
class Attn_Softmax(Function):
    ...
    def forward(ctx: Context, inp: Tensor, mask: Tensor) -> Tensor:
        output = inp.f.attn_softmax_fw(inp, mask)
        ctx.save_for_backward(output, mask)
        return output

    @staticmethod
    def backward(ctx: Context, out_grad: Tensor):
        soft_out, mask = ctx.saved_values
        grad_input = out_grad.f.attn_softmax_bw(out_grad, soft_out)
...
class LayerNorm(Function):
    ...
    def forward(ctx: Context, inp: Tensor, gamma: Tensor, beta: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_2_1
        res, mean, var = inp.f.layernorm_fw(inp, gamma, beta)
