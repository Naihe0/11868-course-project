...
class CudaKernelOps(TensorOps):
    ...
    def attn_softmax_fw(inp: Tensor, mask: Tensor):
      ...
      is_dec_self_attn = (
        ...
        and mask.shape[0] == 1
        and mask.shape[1] == 1
        and mask.shape[2] == to_len
        and mask.shape[3] == to_len
        )
    ...
    def attn_softmax_bw(out_grad: Tensor, soft_inp: Tensor):
        batch, nhead, from_len, to_len = out_grad.shape
        rows = batch * nhead * from_len
        softmax_len = to_len
        ...
        lib_softmax.launch_attn_softmax_bw.argtypes = [
            ...
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags='C_CONTIGUOUS'), # soft_outp
            ctypes.c_int,                                                        # rows
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
      ...
      out = np.empty(inp._tensor._storage.shape, dtype=datatype)
      var = np.empty((batch_size,), dtype=datatype)
      mean = np.empty((batch_size,), dtype=datatype)
      ...
      return (
        tensor_from_numpy(out.reshape(inp.shape), backend=inp.backend),
        tensor_from_numpy(mean, backend=inp.backend),
        tensor_from_numpy(var, backend=inp.backend),
      )
    ...
    def layernorm_bw(out_grad: Tensor, inp: Tensor, gamma: Tensor, beta: Tensor, var: Tensor, mean: Tensor):
      ...
      gamma_grad = np.empty(gamma._tensor._storage.shape, dtype=datatype)
      beta_grad = np.empty(beta._tensor._storage.shape, dtype=datatype)
      inp_grad = np.empty(inp._tensor._storage.shape, dtype=datatype)
      ...
      return (
        tensor_from_numpy(inp_grad.reshape(inp.shape), backend=inp.backend),
        tensor_from_numpy(gamma_grad.reshape(gamma.shape), backend=gamma.backend),
        tensor_from_numpy(beta_grad.reshape(beta.shape), backend=beta.backend),
      )
