...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    order_list = topological_sort(variable)
    grads = {variable.unique_id: deriv}   # d[variable] = deriv

    for var in order_list:
        d = grads.get(var.unique_id, 0)
        if var.is_leaf():
            var.accumulate_derivative(d)
        else:
            d_chain = var.chain_rule(d)
            for (parent , df) in d_chain:
                grads[parent.unique_id] = grads.get(parent.unique_id, 0) + df
