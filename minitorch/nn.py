...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    softmax_result = logsumexp(logits, dim = 1)
    one_hot_embed = one_hot(target, logits.shape[1])
    correct = (logits * one_hot_embed).sum(dim=1) 
    result = softmax_result - correct
    ### BEGIN ASSIGN3_1
    ### END ASSIGN3_1
