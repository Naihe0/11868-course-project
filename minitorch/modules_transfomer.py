...
class MultiHeadAttention(Module):
    def __init__(self, n_embd: int, n_head: int, causal: bool=True, p_dropout: float=0.1, bias: bool=True, backend: TensorBackend=None):
        ...
        self.q_projection = Linear(n_embd, n_embd,backend=backend,bias=bias )
        self.k_projection =  Linear(n_embd, n_embd,backend=backend,bias=bias)
        self.v_projection =  Linear(n_embd, n_embd,backend=backend,bias=bias)
        self.out_projection = Linear(n_embd, n_embd ,backend=backend,bias=bias)
    ...
    def self_attention(self, q, kT, v):
        ...
        attn = MatMul.apply(q, kT)/q_dim**0.5
        use_fused = getattr(attn.backend, "use_fused_kernel", False)
        # if self.causal:
        ...
        if self.causal:
            causal_mask = self.create_causal_mask(queries_len)
            if use_fused:
                attn = Attn_Softmax.apply(attn, causal_mask)
            else:
                attn = attn + causal_mask
                attn = softmax(attn, dim = 3)
        else:
            attn = Attn_Softmax.apply(attn, None)
        result = MatMul.apply(attn, v)
    ...
    def forward(self, x):
        ...
        out = self.out_projection(out).view(batch_size, seq_len, n_embd)
...
class DecoderLM(Module):
    ...
    def forward(self, idx):
        ...
        ### BEGIN ASSIGN3_3
        tok = self.token_embeddings(idx)
        pos_ids = tensor_from_numpy(
            np.arange(seq_len, dtype=np.int32).reshape(1, seq_len),
        ...
        x = self.lm_head(x.view(batch_size * seq_len, self.n_embd)).view(
