...
__global__ void ker_layer_norm(T *ln_res, T *vars, T *means, const T *inp,
  ...
  // Step 1: Compute sum of x and sum of x^2
  float l_sum = 0;
  ...
  // Step 2: blockReduce to get total sum and square sum, compute mean and variance
  float reduce_val[2] = {l_sum, l_square_sum};
  blockReduce<ReduceType::kSum, 2>(reduce_val);
  __shared__ float s_mean, s_var;
  if (threadIdx.x == 0) {
    // hidden_size is already divided by 4, so total elements = hidden_size * 4
    float total = (float)(hidden_size * 4);
    s_mean = reduce_val[0] / total;
    // variance = E[x^2] - E[x]^2
    s_var = reduce_val[1] / total - s_mean * s_mean + LN_EPSILON;
    // Store mean and var outputs
  ...
  // Step 3: Normalize, apply scale and bias, store result
  ...
  for (uint idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
    ...
    float4 res;
    res.x = (val.x - s_mean) * s_var * s.x + b.x;
    res.y = (val.y - s_mean) * s_var * s.y + b.y;
    res.z = (val.z - s_mean) * s_var * s.z + b.z;
    res.w = (val.w - s_mean) * s_var * s.w + b.w;
    out_f4[idx] = res;
  }
  /// END ASSIGN4_2_1
}
...
__global__ void ker_ln_bw_dgamma_dbetta(T *gamma_grad, T *betta_grad,
  ...
  // Each block handles TILE_DIM columns starting at blockIdx.x * TILE_DIM
  // threadIdx.x indexes within TILE_DIM columns, threadIdx.y indexes rows
  int idx = blockIdx.x * TILE_DIM + threadIdx.x;
  float local_dgamma = 0.0f;
  float local_dbetta = 0.0f;

  // Each thread processes rows: threadIdx.y, threadIdx.y + TILE_DIM, threadIdx.y + 2*TILE_DIM, ...
  for (int row = threadIdx.y; row < rows; row += TILE_DIM) {
    if (idx < width) {
      float dout_val = out_grad[row * width + idx];
      local_dbetta += dout_val;

      // Compute xhat = (inp - mean) * rsqrt(var)
      float xhat;
      if (means != nullptr) {
        xhat = (inp[row * width + idx] - means[row]) * rsqrtf(vars[row] + LN_EPSILON);
      } else {
        xhat = (inp[row * width + idx] - betta[idx]) / gamma[idx];
      }
      local_dgamma += dout_val * xhat;
    }
  }

  // Step 2: Store partial gradients in shared memory
  betta_buffer[threadIdx.x][threadIdx.y] = local_dbetta;
  gamma_buffer[threadIdx.x][threadIdx.y] = local_dgamma;
  __syncthreads();

  // Step 3: Reduce along threadIdx.y dimension using g.shfl_down
  // Each thread reads from shared memory (transposed access)
  float sum_betta = betta_buffer[threadIdx.y][threadIdx.x];
  float sum_gamma = gamma_buffer[threadIdx.y][threadIdx.x];

  for (int i = TILE_DIM / 2; i >= 1; i >>= 1) {
    sum_betta += g.shfl_down(sum_betta, i);
    sum_gamma += g.shfl_down(sum_gamma, i);
  }
  ...
  if (threadIdx.x == 0) {
    ...
    if (col_idx < width) {
      betta_grad[col_idx] = sum_betta;
      gamma_grad[col_idx] = sum_gamma;
    }
  }
  /// END ASSIGN4_2_2
}
...
__global__ void ker_ln_bw_dinp(T *inp_grad, const T *out_grad, const T *inp,
  ...
  float rstd = rsqrtf(vars[blockIdx.x] + LN_EPSILON);
  ...
  // Step 1: Compute dxhat = dy * gamma, and accumulate sums
  // Step 2: Compute xhat = (inp - mean) * rstd
  for (uint idx = threadIdx.x; idx < hidden_dim; idx += blockDim.x) {
    ...
    // dxhat = dy * gamma
    float dxhat_x = dy.x * w.x;
    float dxhat_y = dy.y * w.y;
    float dxhat_z = dy.z * w.z;
    float dxhat_w = dy.w * w.w;
    
    l_sum_dxhat += dxhat_x + dxhat_y + dxhat_z + dxhat_w;
    
    // xhat = (x - mean) * rstd
    float xhat_x = (x.x - mean) * rstd;
    float xhat_y = (x.y - mean) * rstd;
    float xhat_z = (x.z - mean) * rstd;
    float xhat_w = (x.w - mean) * rstd;
    
    l_sum_dxhat_xhat += dxhat_x * xhat_x + dxhat_y * xhat_y + dxhat_z * xhat_z + dxhat_w * xhat_w;
  }
  
  // Step 3: blockReduce sums
  float reduce_val[2] = {l_sum_dxhat, l_sum_dxhat_xhat};
  blockReduce<ReduceType::kSum, 2>(reduce_val);
  ...
  // dinp = (dxhat - (sum(dxhat) + xhat * sum(dxhat * xhat)) / hidden_dim_total) * rstd
  float total = (float)(hidden_dim * 4);
  ...
  for (uint idx = threadIdx.x; idx < hidden_dim; idx += blockDim.x) {
    ...
    dxhat = dy.x * w.x;
    xhat = (x.x - mean) * rstd;
    res.x = (dxhat - (s_sum_dxhat + xhat * s_sum_dxhat_xhat) / total) * rstd;
    
    dxhat = dy.y * w.y;
    xhat = (x.y - mean) * rstd;
    res.y = (dxhat - (s_sum_dxhat + xhat * s_sum_dxhat_xhat) / total) * rstd;
    
    dxhat = dy.z * w.z;
    xhat = (x.z - mean) * rstd;
    res.z = (dxhat - (s_sum_dxhat + xhat * s_sum_dxhat_xhat) / total) * rstd;
    
    dxhat = dy.w * w.w;
    xhat = (x.w - mean) * rstd;
    res.w = (dxhat - (s_sum_dxhat + xhat * s_sum_dxhat_xhat) / total) * rstd;
