    ...
    const int *b_strides)
{
  ...
  // 1. Compute the row and column of the output matrix this thread will compute
  int row = blockIdx.x * TILE + tx;
  int col = blockIdx.y * TILE + ty;
  
  // Get matrix dimensions
  int m = a_shape[1];  // rows of A and C
  int n = a_shape[2];  // cols of A, rows of B
  int p = b_shape[2];  // cols of B and C
  
  // 2. Initialize accumulator for the output element
  float sum = 0.0f;
  
  // 3. Iterate over tiles of the two input matrices
  ...
  for (int t = 0; t < num_tiles; t++) {
    ...
    // 6. Synchronize to make sure all threads are done before loading next tile
    __syncthreads();
  }
  
  // 7. Write the output to global memory
  if (row < m && col < p) {
    out[batch * out_strides[0] + row * out_strides[1] + col * out_strides[2]] = sum;
  }
  /// END HW1_4
}
    ...
    int fn_id)
{
  ...
  if (out_pos < out_size) {
    ...
    // Calculate the output storage position
