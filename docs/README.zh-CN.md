...
| `project/run_gpu_kv_benchmark.py` | 直接调用 CUDA runtime，测量 GPU-resident KV cache 的 decode attention microbenchmark。 |
...
| `project/plot_gpu_kv_results.py` | 绘制 GPU-only KV cache microbenchmark 图。 |
...
### GPU-only KV cache microbenchmark

`project/run_gpu_kv_benchmark.py` 是一个更聚焦的 CUDA runtime microbenchmark。它不走完整 `PagedDecoderLM`，而是直接使用 `PagedAttentionKernel` 的 stateful runtime：先把 K/V cache 上传到 GPU runtime buffer 一次，然后在计时循环里只调用 decode attention，让 kernel 从 device-resident K/V cache 读取数据。对比项是旧的 stateless wrapper 路径，它每次 attention call 都重新复制完整 host KV cache。
...
python project/plot_gpu_kv_results.py
```
...
GPU-only KV cache microbenchmark 和绘图：
