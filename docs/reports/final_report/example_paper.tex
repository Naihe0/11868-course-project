...
\begin{itemize}
    ...
    \item \textbf{RQ5:} Do the same memory tradeoffs hold when the full pretrained GPT-2 model runs through MiniTorch with GPU-resident KV caches?
\end{itemize}
...
Figure~\ref{fig:timing} reports decode and prefix timing from the earlier implementation.  In that version, the benchmark was still dominated by MiniTorch/Python overhead and non-final cache-update plumbing, so the paged decode curve does not show a clean speedup over the no-cache baseline.  This is not the result we use to claim GPU-resident KV behavior; it is a historical timing experiment that motivated the later GPT-2 refactor.  The main lesson from Figure~\ref{fig:timing} is narrower: allocator wins do not automatically imply kernel speedups when the runtime path is not yet optimized.
...
\caption{Earlier timing experiments separate allocator/caching behavior from CUDA/kernel quality.  Figure 3 predates the final GPU-only KV-cache path, which is why it should not be interpreted as the final no-cache versus paged speed comparison.  Error bars show min--max across timed trials where available.}
\label{fig:timing}
\end{figure*}

\subsection{Pretrained GPT-2 Full-Model Benchmark}

The pretrained GPT-2 benchmark is the post-refactor experiment: it was run after changing the MiniTorch path so contiguous and paged KV caches live on the GPU and the timed cache updates avoid CPU/GPU KV transfer.  Hugging Face is used only to load the checkpoint, tokenizer, and WikiText-2 text stream; the timed baseline, contiguous-KV, and paged-attention paths all run the full MiniTorch transformer.  The paged and contiguous cache implementations own GPU-resident K/V storage and update slots with device-to-device copies, so the timed KV path reports zero host-to-device KV bytes.  We use batch size 1, block size 16, prompt lengths 14, 30, and 62, a static contiguous cap of 128 tokens, one warmup, and three timed iterations.
...
\bottomrule
\end{tabular}
\caption{Pretrained GPT-2 full-model MiniTorch benchmark on WikiText-2.  Baseline recomputes the full context and has no persistent KV cache; contiguous and paged variants cache K/V entirely on the GPU.  All paged rows matched contiguous outputs with zero max error and reported \texttt{timed\_h2d\_kv\_bytes=0}.}
...
\textbf{Memory gains are stronger than speed gains in this prototype.}  Paged allocation dramatically improves KV efficiency and fixed-budget capacity, including in the post-refactor pretrained GPT-2 MiniTorch run where it cuts reserved KV memory by 50--87.5\% versus a 128-token contiguous slab while keeping timed KV-cache updates on the GPU.  Figure~\ref{fig:timing} comes from the older implementation, so its weak no-cache comparison should be attributed to the earlier runtime path rather than to the final GPU-only KV-cache design.  Even in the newer GPT-2 run, paged attention is close to but not consistently faster than contiguous KV.  This is expected for a batch-size-one latency benchmark: contiguous KV has direct addressing, while paged KV adds block-table indirection.  The system-level win should appear as higher admission capacity or larger effective batches under a fixed memory budget, not necessarily lower single-request decode latency.  This matches the project goal: validate the systems abstraction in a compact framework, not reproduce vLLM throughput.
